...
def euclidean(board, player):
  ...
  for index, value in np.ndenumerate(board):
    if value == player:
      s = s + distance.euclidean(index, goals[player])
  return round(s, 2)
...
def v_displacement(board, player):
  ...
  for index, value in np.ndenumerate(board):
    if value == player:
      me = me + distance.euclidean(index, goals[player])
    if value == other(player):
      opponent = opponent + distance.euclidean(index, goals[player])
    ...
    if value == player:
      ...
      for jndex, value in np.ndenumerate(board):
        ...
        if((x == BLUE_PLAYER or x == RED_PLAYER) and x==y):
          s = s+0.1*distance.euclidean(index, jndex)
  return round(s, 2)